...
> 当前状态：持续开发中
> 当前主分支：`main`
> 当前功能分支：`codex/add-historical-etf-universe`
...
后续已合并：`minimum_history`、真实ETF数据、沪深300 Benchmark、样本外验证、回撤
重新入场和V6.1单因素研究。当前 `codex/fix-audit-logic-issues` 统一修复跨实现审计问题。
...
- 已完整平仓持仓周期的平均持仓时间
- 相同数据与配置下的可重复性验证
- `minimum_history`逐标的暖机资格
- 沪深300 Benchmark、超额收益、跟踪误差和信息比率
- Walk Forward、Rolling与牛/熊/震荡分段验证
- 持仓缺行情和无成交价的失败即停止门禁

尚未实现或尚未完成：

- 历史时点完整ETF池及退市、合并、更名数据
- 停牌估值、涨跌停、部分成交与成交量容量规则
- 滑点参数的真实市场校准

## 四、当前策略与仓库结构

仓库包含三套用途不同的实现，不得视为等价：

1. `etf-rotation-v6`：唯一策略、参数、回测和研究结论真源。
2. 根目录 JavaScript：独立实时ETF雷达，不得引用V6绩效解释建议。
3. `ETF-Quant-System`：历史兼容框架，不得把输出标记为V6。
...
并保存获取清单、实际区间、每标的行数和文件哈希。工程已支持权威历史 ETF 目录、
点时成员资格、退市行情覆盖与证据清单门禁；当前仍缺获授权的全量目录和对应行情，
不能把固定研究池描述为无幸存者偏差。
...
已实施固定参数 Walk Forward、Rolling 以及牛市、熊市、震荡市分阶段验证，并修复
`DrawdownGuard` 冷却后立即重复触发的问题。复验显示牛市有效、熊市显著亏损且完整区间
最大回撤过高。记录见 `etf-rotation-v6/docs/out_of_sample_validation.md`。

### Phase 8：V6.1（单因素研究完成，未晋级）

已仅研究防守状态风险暴露一个因素。开发期选择的 0% 候选在最终留出期的收益、最大
回撤、Sharpe 和 Calmar 均弱于 V6 基准，因此不发布 V6.1，`strategy_v6.yaml` 保持
不变。记录见 `etf-rotation-v6/docs/v6_1_research.md`。
...
- 工程成熟度：较高；真实数据、Benchmark、样本外验证和单因素研究链路均已建立。
- 回测可信度：成交时序、漂移、成本、事件、暖机、基准和验证边界均有测试；历史池
  工程门禁已完成，但全量授权目录、停牌/退市行情和真实市场冲击数据仍未接入。
- 策略可信度：具备固定研究池上的真实历史证据，但熊市损失和最大回撤过高，V6.1
  候选未通过最终留出验证，不适合进入模拟盘或实盘。
- 当前主要矛盾：策略在牛市表现与熊市防守之间失衡，且当前最终留出期已经使用，不能
  继续用它搜索参数。
- 下一阶段重点：补齐无幸存者偏差的历史 ETF 池与交易可行性规则，并积累新的未查看
  样本外数据；在此之前不继续增加策略复杂度。

> 当前研究不构成投资建议；真实历史结果也不得直接用于实盘决策。
